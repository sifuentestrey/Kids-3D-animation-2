...
// removing creatures/toys doesn't leak memory over a long kiosk session.
...
export const ThreeCanvas: React.FC<ThreeCanvasProps> = ({
  ...
  // Last position/rotation explicitly pushed from React state per creature,
  // so autonomous wander/bounce motion (applied directly to the group in the
  // render loop) isn't stomped when an unrelated creature/toy re-syncs.
  ...
  useEffect(() => {
    ...
    creatures.forEach((c) => {
      ...
      if (!group) {
        ...
        // rather than replace userData so an unrelated update doesn't wipe an
        // in-progress wander target, and only snap position/rotation when
        // React's copy actually changed (a real drive/ground-click command),
        // not just because some other creature's state changed.
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    // A finger drag is a camera orbit, not a tap — skip the click raycasts.
    if (wasDragging && dragDistanceRef.current > CLICK_DRAG_THRESHOLD) {
      return;
    }
...
function buildKidsLandEnvironment(
  ...
  while (envGroup.children.length > 0) {
    const decorChild = envGroup.children[0];
    envGroup.remove(decorChild);
    disposeObject3D(decorChild);
  }
...
function buildRiggedCharacter(
  ...
  while (rootGroup.children.length > 0) {
    const rigChild = rootGroup.children[0];
    rootGroup.remove(rigChild);
    disposeObject3D(rigChild);
  }
